...
void safe_swap()
{
    ...
    std::swap(a, b);
    // 错误用法
    // mtx1 mtx2 都交给lock1和lock2管理，解锁操作就无法通过互斥量完成
    // mtx1.unlock();
    // mtx2.unlock();
}
...
void use_return()
{
    ...
    lock 使用移动构造函数来接收这个右值。
    lock 接管了 get_lock()返回的临时 std::unique_lock<std::mutex>对象的所有权，
    并且 mtx 仍然保持锁定状态。

    在新的 lock 对象的保护下递增 shared_data。
    由于 lock 在作用域内锁定了 mtx，是线程安全的。
    */
}
